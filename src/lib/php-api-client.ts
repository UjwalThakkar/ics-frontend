...
class PHPAPIClient {
  ...
  async getAvailableSlots(date: string, service: string) {
    return this.request<any>(`/appointments/slots?date=${date}&service=${service}`)
  }
  ...
  admin = {
    ...
    bulkUpdateApplications: (applicationIds: string[], action: string, notes?: string) =>
      this.request<any>('/admin/applications/bulk-update', {
        method: 'POST',
        body: JSON.stringify({ application_ids: applicationIds, action, notes })
      }),
    ...
    getAnalytics: (params: { period: string; type: string; service?: string }) => {
      const queryString = new URLSearchParams(params as any).toString()
