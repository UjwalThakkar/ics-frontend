...
const Services = () => {
    ...
    const fetchServices = async () => {
      try {
        ...
        normalizedServices.sort((a, b) => {
          const orderDiff = (apiServices.find(s => s.service_id === a.id)?.display_order || 0) -
                           (apiServices.find(s => s.service_id === b.id)?.display_order || 0)
        ...
        const categories = Array.from(new Set(normalizedServices.map(s => s.category)))
          .filter(Boolean)
          .sort()
  ...
  const formatFee = (fees: Service['fees']) => {
    ...
    const standard = fees.find(f => f.description.toLowerCase() === 'standard')
    if (!standard) return `${fees[0].currency} ${fees[0].amount}+`
    return standard.amount === 0 ? 'No fee' : `${standard.currency} ${standard.amount}`
  }
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Comprehensive consular services for Indian nationals and foreign citizens.
          </p>
        </div>
        ...
        <div className="mb-8 flex flex-wrap justify-center gap-2">
          ...
          {serviceCategories.map(cat => {
            const count = services.filter(s => s.category === cat && s.isActive).length
            return (
              <button
                key={cat}
