// src/components/admin/services/EditServiceModal.tsx
import { X, Loader2 } from "lucide-react";
import { useState, useEffect, useCallback } from "react";
import { Service, ServiceFee } from "@/types/admin";
import { phpAPI } from "@/lib/php-api-client";

interface FeeRow {
  type: string;
  amount: string;
}

interface EditForm {
  category?: string;
  title?: string;
  description?: string;
  processing_time?: string;
  required_documents?: string;
  eligibility_requirements?: string;
  display_order?: number | "";
  is_active?: 0 | 1;
}

interface Props {
  serviceId: number;
  onClose: () => void;
  onUpdate: () => void;
}

export default function EditServiceModal({
  serviceId,
  onClose,
  onUpdate,
}: Props) {
  const [loading, setLoading] = useState(true);
  const [form, setForm] = useState<EditForm>({});
  const [fees, setFees] = useState<FeeRow[]>([]);

  const memoizedOnClose = useCallback(onClose, [onClose]);

  /* -------------------------------------------------------------
   *  Load service
   * ------------------------------------------------------------- */
  useEffect(() => {
    const fetch = async () => {
      setLoading(true);
      try {
        const { service: s } = await phpAPI.admin.getService(serviceId);

        setForm({
          category: s.category ?? "",
          title: s.title ?? "",
          description: s.description ?? "",
          processing_time: s.processing_time ?? "",
          required_documents: Array.isArray(s.required_documents)
            ? s.required_documents.join(", ")
            : "",
          eligibility_requirements: Array.isArray(s.eligibility_requirements)
            ? s.eligibility_requirements.join(", ")
            : "",
          display_order: s.display_order ?? "",
          is_active: s.is_active ? 1 : 0,
        });

        // Populate fee rows – keep empty row at the end for quick add
        const feeRows =
          Array.isArray(s.fees) && s.fees.length
            ? s.fees.map((f) => ({
                type: f.type ?? "",
                amount: String(f.amount ?? ""),
              }))
            : [{ type: "", amount: "" }];

        setFees(feeRows);
      } catch {
        alert("Failed to load service");
        memoizedOnClose();
      } finally {
        setLoading(false);
      }
    };
    fetch();
  }, [serviceId, memoizedOnClose]);

  /* -------------------------------------------------------------
   *  Fee helpers
   * ------------------------------------------------------------- */
  const addFeeRow = () => setFees((prev) => [...prev, { type: "", amount: "" }]);

  const removeFeeRow = (idx: number) =>
    setFees((prev) => prev.filter((_, i) => i !== idx));

  const updateFeeRow = (idx: number, field: "type" | "amount", value: string) =>
    setFees((prev) =>
      prev.map((r, i) => (i === idx ? { ...r, [field]: value } : r))
    );

  /* -------------------------------------------------------------
   *  Submit
   * ------------------------------------------------------------- */
  const handleSubmit = async () => {
    try {
      const parsedFees: ServiceFee = fees
        .filter((r) => r.type.trim() && r.amount.trim())
        .map((r) => ({
          type: r.type.trim(),
          amount: Number(r.amount),
        }));

      const requiredDocs = (form.required_documents ?? "")
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);

      const eligibility = (form.eligibility_requirements ?? "")
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);

      const payload: Partial<Service> = {
        title: form.title,
        description: form.description,
        processing_time: form.processing_time,
        fees: parsedFees,
        required_documents: requiredDocs,
        eligibility_requirements: eligibility,
        display_order:
          typeof form.display_order === "number" ? form.display_order : undefined,
        is_active: form.is_active,
        category: form.category,
      };

      await phpAPI.admin.updateService(serviceId, payload);
      onUpdate();
      onClose();
    } catch (err: any) {
      alert(err?.message ?? "Invalid data – check fee rows");
    }
  };

  /* -------------------------------------------------------------
   *  Render
   * ------------------------------------------------------------- */
  if (loading) {
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <Loader2 className="h-8 w-8 animate-spin text-blue-600" />
      </div>
    );
  }

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-screen overflow-y-auto">
        <div className="flex justify-between items-center p-6 border-b">
          <h2 className="text-xl font-bold">Edit Service</h2>
          <button onClick={onClose} className="text-gray-500 hover:text-gray-700">
            <X className="h-5 w-5" />
          </button>
        </div>

        <div className="p-6 space-y-4">
          <input placeholder="Category" value={form.category ?? ""}
            onChange={(e) => setForm((f) => ({ ...f, category: e.target.value }))}
            className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" />
          <input placeholder="Title" value={form.title ?? ""}
            onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))}
            className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" />
          <textarea placeholder="Description" rows={3} value={form.description ?? ""}
            onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))}
            className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" />
          <input placeholder="Processing Time" value={form.processing_time ?? ""}
            onChange={(e) => setForm((f) => ({ ...f, processing_time: e.target.value }))}
            className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" />

          {/* ---- NEW: Fees table ---- */}
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="block text-sm font-medium text-gray-700">Fees</label>
              <button type="button" onClick={addFeeRow}
                className="text-xs text-blue-600 hover:text-blue-800">
                + Add fee
              </button>
            </div>

            <div className="space-y-2">
              {fees.map((row, idx) => (
                <div key={idx} className="flex gap-2 items-center">
                  <input
                    placeholder="Type"
                    value={row.type}
                    onChange={(e) => updateFeeRow(idx, "type", e.target.value)}
                    className="flex-1 px-3 py-1 border rounded-md text-sm"
                  />
                  <input
                    type="number"
                    placeholder="Amount"
                    value={row.amount}
                    onChange={(e) => updateFeeRow(idx, "amount", e.target.value)}
                    className="w-24 px-3 py-1 border rounded-md text-sm"
                  />
                  <button
                    type="button"
                    onClick={() => removeFeeRow(idx)}
                    className="text-red-600 hover:text-red-800"
                  >
                    <X className="h-4 w-4" />
                  </button>
                </div>
              ))}
            </div>
          </div>

          <input placeholder="Required Documents (comma-separated)" value={form.required_documents ?? ""}
            onChange={(e) => setForm((f) => ({ ...f, required_documents: e.target.value }))}
            className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" />
          <input placeholder="Eligibility (comma-separated)" value={form.eligibility_requirements ?? ""}
            onChange={(e) => setForm((f) => ({ ...f, eligibility_requirements: e.target.value }))}
            className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" />
          <input type="number" placeholder="Display Order" value={form.display_order ?? ""}
            onChange={(e) => {
              const v = e.target.value;
              setForm((f) => ({
                ...f,
                display_order: v === "" ? "" : Number(v),
              }));
            }}
            className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" />
          <label className="flex items-center space-x-2">
            <input type="checkbox" checked={!!form.is_active}
              onChange={(e) => setForm((f) => ({ ...f, is_active: e.target.checked ? 1 : 0 }))}
              className="h-4 w-4" />
            <span>Active</span>
          </label>
        </div>

        <div className="flex justify-end space-x-3 p-6 border-t">
          <button onClick={onClose}
            className="px-4 py-2 border rounded-lg hover:bg-gray-50">
            Cancel
          </button>
          <button onClick={handleSubmit}
            className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
            Update Service
          </button>
        </div>
      </div>
    </div>
  );
}